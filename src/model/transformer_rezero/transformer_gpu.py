...
class Transformer(t.nn.Module):
    ...
    def recognize(self, feature, feature_length, beam=5, penalty=0, ctc_weight=0.3, maxlenratio=0,
        ...
        if lpz is not None:
            ...
            ctc_prefix_score = CTCPrefixScore(lpz.cpu().detach().numpy(), self.vocab.blank_id, self.vocab.eos_id, np)
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                ys_mask = Masker.get_mask(t.LongTensor([i+1])).cuda()
                ...
                ys = t.tensor(hyp['yseq']).unsqueeze(0).cuda()
                ...
                if lpz is not None:
                    ...
                    local_scores = \
                        ...
                        + ctc_weight * t.from_numpy(ctc_scores - hyp['ctc_score_prev']).cuda()
                    if rnnlm:
