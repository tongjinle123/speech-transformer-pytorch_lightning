...
from src.model.transformer_lm.lightning_model import LightningModel
...
def main(hparams):
    ...
    logger = TestTubeLogger(exp_root, name=log_folder, version=2001)
    checkpoint = ModelCheckpoint(filepath='exp/lightning_logs/version_2001/checkpoints/',
                                 monitor='val_loss', verbose=1, save_top_k=-1)
    trainer = Trainer(
        logger=logger,
