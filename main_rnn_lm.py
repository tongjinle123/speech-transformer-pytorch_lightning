...
from src_test.model.rnn_lm.lightning_model import LightningModel
...
def main(hparams):
    ...
    logger = TestTubeLogger(exp_root, name=log_folder, version=5003)
    checkpoint = ModelCheckpoint(filepath='exp/lightning_logs/5003/checkpoints/',
                                 monitor='val_acc', verbose=1, save_top_k=-1)
    trainer = Trainer(
        logger=logger,
