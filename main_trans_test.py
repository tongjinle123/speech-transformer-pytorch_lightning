...
def main(hparams):
    ...
    logger = TestTubeLogger(exp_root, name=log_folder, version=5000)
    checkpoint = ModelCheckpoint(filepath='exp/lightning_logs/5000/checkpoints/',
                                 monitor='val_loss', verbose=1, save_top_k=-1, mode='min')
    trainer = Trainer(
        logger=logger,
